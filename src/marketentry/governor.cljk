(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether an Algeria-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Algeria-resident-rep missing -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-dz-entity?
                                       true` (near-universal for Algeria
                                       ...
                                       Grounded in Algerian legal entity
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(NIF/統一資格/marchés publics登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- dz-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-dz-entity? true`, INDEPENDENTLY verify
  `:has-dz-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-dz-entity?` ground truth (most Algeria public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-dz-entity? e))
                 (not (true? (:has-dz-entity? e))))
        [{:rule :dz-entity-missing
          :detail (str subject " はアルジェリア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (dz-entity-missing-violations request st)
